class StatusTaskToolbar extends HTMLElement {
  connectedCallback() {
    ...
    if (variant === "downloads") {
      ...
      this.innerHTML = `
        <button id="status-detail-btn" type="button" class="task-toolbar-btn task-toolbar-btn-compact secondary" aria-label="Chi tiết tác vụ" title="Chi tiết tác vụ">
        ...
        <a id="reader-btn" class="button-link secondary disabled task-toolbar-btn task-toolbar-btn-result hidden" href="#" aria-label="Đọc đối chiếu" title="Đọc đối chiếu" aria-disabled="true">
          <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
        ...
        <a id="pdf-btn" class="button-link secondary disabled task-toolbar-btn task-toolbar-btn-result hidden" href="#" target="_blank" rel="noopener noreferrer" aria-label="Tải xuống PDF" title="Tải xuống PDF">
          <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    this.innerHTML = `
      <button id="cancel-btn" type="button" class="task-toolbar-btn secondary" aria-label="Hủy tác vụ" title="Hủy tác vụ" disabled>
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M8.4 8.4l7.2 7.2M15.6 8.4l-7.2 7.2" stroke="currentColor" stroke-width="1.65" stroke-linecap="round"/>
        </svg>
        <span>Hủy</span>
      </button>
      <button id="stop-btn" type="button" class="task-toolbar-btn secondary" aria-label="Dừng thăm dò" title="Dừng thăm dò">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <rect x="7.7" y="7.7" width="8.6" height="8.6" rx="2.2" fill="currentColor"/>
        </svg>
        <span>Dừng</span>
      </button>
      <button id="back-home-btn" type="button" class="task-toolbar-btn secondary hidden" aria-label="Trở về trang chủ" title="Trở về trang chủ">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
